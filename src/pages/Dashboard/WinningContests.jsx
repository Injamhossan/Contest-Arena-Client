        ...
        <DashboardLayout>
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="p-6 border-b border-gray-200 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-gray-900">Winning Contests</h2>
                ...
                {winningContests.length === 0 ? (
                    <div className="p-12 text-center h-64 flex flex-col items-center justify-center">
                        ...
                        <p className="text-gray-500 mt-1">Keep participating to win awesome prizes!</p>
                        <a href="/all-contests" className="mt-4 text-[#4a37d8] font-medium hover:underline">
                            Browse Contests
                        </a>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
                        {winningContests.map((item) => (
                            <div key={item._id} className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow group">
                                ...
                                <div className="p-4">
                                    <h3 className="font-bold text-gray-900 text-lg mb-1">{item.contest?.name}</h3>
                                    <div className="flex items-center text-gray-500 text-sm mb-4">
                                        <Calendar className="w-4 h-4 mr-2" />
                                        {new Date(item.contest?.deadline).toLocaleDateString()}
                                    </div>
                                    <div className="pt-4 border-t border-gray-100 flex justify-between items-center">
                                        <span className="text-sm text-gray-600">Prize Won</span>
                                        <span className="font-bold text-amber-600 text-lg">${item.contest?.prizeMoney}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
